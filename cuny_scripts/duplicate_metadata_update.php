...
function process_duplicate_resource(string $full_path, array $duplicates): void
{
	...
	// delete collections if empty and if it's not a parent
	if (!empty($col_refs)) {
		$i = 0;
	    foreach ($col_refs as $row) {
	        $col_ref = $row['value'];
			$query = "SELECT collection FROM collection_resource WHERE collection = ?;";
			$refs_r = ps_query($query, ['i', $col_ref]);
			
			$query = "SELECT ref FROM collection WHERE parent = ?;";
			$refs_c = ps_query($query, ['i', $col_ref]);
			
			if (empty($refs_r) && empty($refs_c)) {
				$query = "DELETE FROM collection WHERE ref = ?;";
				ps_query($query, ['i', $col_ref]);
				echo "Deleted empty and childless collection: $col_ref\n";
			}
	    }
	}
