...
function check_similarity($input, $shows) {
    ...
    foreach ($shows as $title => $abbr) {
        if (strlen($input) <= 6) {
            similar_text(mb_strtolower($input), mb_strtolower($abbr), $percent);
        } else {
			similar_text(mb_strtolower($input), mb_strtolower($title), $percent);
        }
...
// Example usage
// print_media_dict();
//echo check_similarity("NOSHOW", $shows_dict) . "\n";
